...
def boto3client(assumed_role_arn):
    if assumed_role_arn != '':
        session_name = 'AEM-Stack_Manager'
        sts = boto3.client('sts')
        sts_response = sts.assume_role(RoleArn=assumed_role_arn, RoleSessionName=session_name)
        boto3session = boto3.session.Session(aws_access_key_id=sts_response['Credentials']['AccessKeyId'], aws_secret_access_key=sts_response['Credentials']['SecretAccessKey'], aws_session_token=sts_response['Credentials']['SessionToken'])
        ec2client = boto3session.client('ec2')
        ssmclient = boto3session.client('ssm')
    else:
...
def instance_ids_by_tags(filters, ec2):
...
def deploy_artifact(message, ssm_common_params, ec2, ssm):
    ...
    details = {
        'InstanceIds': instance_ids_by_tags(target_filter, ec2),
    ...
    return send_ssm_cmd(params, ssm)


def deploy_artifacts(message, ssm_common_params, ec2, ssm):
    target_filter = [
        {
            'Name': 'tag:StackPrefix',
            'Values': [message['stack_prefix']]
...
def export_package(message, ssm_common_params, ec2, ssm):
    target_filter = [
        {
            'Name': 'tag:StackPrefix',
            'Values': [message['stack_prefix']]
...
def export_packages(message, ssm_common_params, ec2, ssm):
    target_filter = [
        {
            'Name': 'tag:StackPrefix',
            'Values': [message['stack_prefix']]
...
def import_package(message, ssm_common_params, ec2, ssm):
    target_filter = [
        {
            'Name': 'tag:StackPrefix',
            'Values': [message['stack_prefix']]
...
def promote_author(message, ssm_common_params, ec2, ssm):
    target_filter = [
        {
            'Name': 'tag:StackPrefix',
            'Values': [message['stack_prefix']]
...
def enable_crxde(message, ssm_common_params, ec2, ssm):
    target_filter = [
        {
            'Name': 'tag:StackPrefix',
            'Values': [message['stack_prefix']]
...
def disable_crxde(message, ssm_common_params, ec2, ssm):
    target_filter = [
        {
            'Name': 'tag:StackPrefix',
            'Values': [message['stack_prefix']]
...
def run_adhoc_puppet(message, ssm_common_params, ec2, ssm):
    target_filter = [
        {
            'Name': 'tag:StackPrefix',
            'Values': [message['stack_prefix']]
...
def live_snapshot(message, ssm_common_params, ec2, ssm):
    target_filter = [
        {
            'Name': 'tag:StackPrefix',
            'Values': [message['stack_prefix']]
...
def sns_message_processor(event, context):
    ...
    for record in event['Records']:
        assumed_role = record['role_arn']
        ec2, ssm = boto3client(assumed_role)
